...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 流单确定
      FlowListT() {
        if(!this.reasonForm.reason && this.reasonForm.reason != 0){
          this.$message({
            message: '请选择流单原因',
            type: 'warning'
          });
          ...
          this.$get( 'crmManagement/item/editState',{id: this.form.id, stage: this.stage, reason: this.reasonForm.reason,reason_brief: this.reasonForm.reason_brief})
            .then ( (data) => {
              if(data.code){
                this.$message({
                  message: data.errorMsg,
