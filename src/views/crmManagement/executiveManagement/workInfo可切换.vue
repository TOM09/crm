                  ...
                  >
                   <template slot-scope="scope">
                     <span :class="{tableInfo: scope.row.log_status_str === '待启动', 
                        ...
                        tablegreen: ((scope.row.log_status_str === '初稿定稿') || (scope.row.log_status_str === '修改稿定稿') || (scope.row.log_status_str === '已完成'))
                        }">
            ...
            <el-dialog title="新增附件" :visible.sync="newFileAdd" :modal="false" width="600px">
              <el-form label-position='right' label-width="120px" :model="r_data">
                ...
                <el-form-item label="备注说明 :">
                  <el-input type="textarea" :rows="5" v-model="r_data.remarks" placeholder="请输入内容" class='inputWarn2'></el-input>
                </el-form-item>
              </el-form>
              ...
              <p class='CustomerLeaderOne'>执行明细{{changeExecutor.number}}</p>
                ...
                <div v-if="changeExecutor.service_type == 1">
                  <el-form-item label="执行负责人 :" class='is-required' 
					           prop="textName4"                 
                  >
                    <el-select class="inputWarn" clearable placeholder="可选择一位执行负责人" v-model="changeExecutor.excutor_leading" filterable>
                        <el-option
                          v-for="item in person"
                          :key="item.dd_id"
                          :label="item.nickname"
...
<!-- 执行明细跟进 -->
    <el-dialog  :visible.sync="executorChange4" title="执行情况跟进" :modal="false" width="550px" class='executiveLogging3'>
  ...
  data() {
    return {
      ...
      executorChange4DataArray:[
        { text: '待启动', value: '待启动' },
        { text: '进行中', value: '进行中' },
        { text: '初稿定稿', value: '初稿定稿' },
        { text: '修改中', value: '修改中' },
        { text: '修改稿定稿', value: '修改稿定稿' },
        { text: '已完成', value: '已完成' },
        { text: '已暂停', value: '已暂停' },
        { text: '已退款', value: '已退款' }
        ],
