...
<!-- 下部详情 -->
    <div class="workInfo_content">
      <el-tabs value="first"  @tab-click='workOperateLog2' class="workOperateLog22">
              ...
              <!-- <table cellspacing="0" cellpadding="0" class='Execution_detail_table' v-if='item.type == 1'>
               ...
               <table cellspacing="0" cellpadding="0" class='Execution_detail_table'>
                <tr class='Execution_detail_table_tr'>
                  ...
                  <th>退款时间</th>
                  <th>退款金额</th>
                  <th>执行记录 <i class="el-icon-view" @click='executiveLeyeOpen(item.id)'></i></th>
                </tr>
                <tr class='Execution_detail_table_tr2'>
                  <td>{{item.begin_time}}</td>
                  <td>{{item.confirm_time}}</td>
                  <td>{{item.mod_count}}</td>
        ...
        <el-tab-pane :label="'预警'+ warn_count" name="fourth">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
            ...
            <el-table ref="multipleTable" :data="warnListData" tooltip-effect="dark" style="width: 100%"  class='multipleTableColor3' align="center">
                 ...
                 <el-table-column type="expand">
                  <template slot-scope="props">
                    <el-table label-position="right" inline class="demo-table-expand"  style="width: 60%" :data="warnListData">
                      <el-table-column prop="number" label="解决方式">
                        </el-table-column>
                      <el-table-column prop="create_time" label="解决时间">
                        </el-table-column>

                      <el-table-column prop="ae_nickname" label="解决人">

                      </el-table-column>
                      <!-- <el-form-item label="解决方式">
                        <span>已核实打款，确认进账</span>
                      </el-form-item>
                      <el-form-item label="解决时间">
                        <span>2018-10-19 09:13</span>
                      </el-form-item>
...
<!-- 执行记录小眼睛 -->
<el-dialog :visible.sync="executiveLoggingEye" class='multipleTableColor multipleTableColor2' title="执行记录" :modal="false" width="90%">
...
<!-- 客服转单记录 -->
<el-dialog  :visible.sync="executiveLogging2"  title="转单记录(客服)"  class='multipleTableColor' :modal="false" width="90%">
       <el-card class="box-card">
            <div class="block">
                <span class="demonstration">
                    <el-table ref="multipleTable" :data="changeRecord" tooltip-effect="dark" style="width: 100%"  align="center">
                        <el-table-column prop="before_excutor_leading" label="转出前执行负责人"></el-table-column>
                        <el-table-column prop="before_excutor" label="转出前执行人"></el-table-column>
                        <el-table-column prop="after_excutor_leading" label="转出后执行负责人"></el-table-column>
...
<!-- 更换客服 -->
<el-dialog  :visible.sync="workInReplace" title="更换客服" :modal="false" width="40%" class='executiveLogging3'>
...
<!-- 关闭预警 -->
<el-dialog  :visible.sync="handleWarningDialog" title="关闭预警" :modal="false" width="40%">
...
<!--执行工单中的四个按钮 -->
      <el-dialog  :visible.sync="executorFour"  title="转单记录"  class='multipleTableColor' :modal="false" width="90%">
       <el-card class="box-card">
            <div class="block">
                <span class="demonstration">
            ...
            <div>
              <p class='CustomerLeaderOne'>执行明细{{changeExecutor.number}}</p>
                ...
                <el-form-item label="执行方 :" class='is-required'>
                  ...
                  <el-form-item label="执行负责人 :" class='is-required'  v-if="changeExecutor.service_type == 1"
					           prop="textName4"                 
                  >
                    <el-select class="inputWarn" placeholder="可选择一位执行负责人" v-model="changeExecutor.excutor_leading" filterable>
                        <el-option
                          v-for="item in person"
                          :key="item.dd_id"
                          :label="item.nickname"
...
<!-- 拆分 -->
        <el-dialog  :visible.sync="executorChange3"  title="执行拆分" :modal="false" width="40%" class='chaifen'>
            <div class='CustomerLeaderAllbox'>
                ...
                <div v-if="item.service_type == 1">
                  <el-form-item label="执行负责人 :" class='is-required'  
                    ...
                    :prop="'excutor_detail.' + index + '.excutor_leading'">
                      <el-select class="inputWarn" value-key='dd_id' placeholder="可选择一位执行负责人" v-model="item.excutor_leading" filterable>
                          <el-option
                            v-for="item in person"
                            :key="item.dd_id"
                            :label="item.nickname"
...
<!-- v-for="(domain, index) in item.excutor_detail.excutor_id"
                ...
                <div v-if = "item.service_type == 2">
                  ...
                  :prop="'excutor_detail.' + index + '.excutor_leading'">
                    <el-select class="inputWarn" placeholder="可选择一位服务商负责人" v-model="item.excutor_leading" filterable>
                        <el-option
                          v-for="item in person"
                          :key="item.dd_id"
                          :label="item.nickname"
                  ...
                  <el-form-item label="服务商 :" class='is-required'
                   ...
                   :prop="'excutor_detail.' + index + '.provider'">                     
                    <el-select class="inputWarn" placeholder="可选择一个服务商" v-model="item.provider" filterable>
                        <el-option
    ...
    <el-dialog  :visible.sync="executorChange4" title="执行情况跟进" :modal="false" width="500px" class='executiveLogging3'>
      ...
      <el-form :model="replaceform" label-width="100px" class='executiveLogging3Top' ref='replaceform' :rules="rules">
        <el-form-item label="执行进度 :" class='is-required'>
          ...
          <el-form-item label="初稿时间 :" class="is-required"  prop="begin_time" v-if="followUPNum == 3">
            <el-date-picker
                class="inputWarn"
          ...
          <el-form-item label="定稿时间 :" class="is-required"  prop="confirm_time" v-if="followUPNum == 5">
            <el-date-picker
                class="inputWarn"
...
export default {
  ...
  methods: {
    ...
    executorChange2f(id){
      ...
      this.changeExecutor.detail_id = id;
...
//更换AE原因
    ...
    executorChange3f(e,id){
      this.zhixingNum = e;
      this.r_dataFrom.detail_id = id;
      ...
      this.$get("manageNew/orderDetail", this.$route.query)
        .then(data => {
          if (data.code) {
            ...
            this.number = findIndex(processArr, objId) + 1;
          } else {
            this.$message({
              message: data.errorMsg,
              type: "warning"
    ...
    // 确定添加附件
    newFileAddT() {
      this.$post("manage/addAttachment", {
        manage_id: this.$route.query.manage_id,
        file: this.file,
        brife: this.brife
      })
